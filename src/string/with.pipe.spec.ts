...
describe('WithPipe', () => {
  ...
  it('Must return boolean true for start & ends #1', () =>
    expect(pipe.transform(text, 'The', 'Reverse')).toEqual(true));
  it('Must return boolean true for start & ends #2', () =>
    expect(pipe.transform(text, 'the', 'reverse')).toEqual(true));
  it('Must return boolean true for start & ends #3', () =>
    expect(pipe.transform(text, 'The', 'Reverse', true)).toEqual(true));

  it('Must return boolean false for start & ends #4', () =>
    expect(pipe.transform(text, 'the', 'reverse', true)).toEqual(false));
  it('Must return boolean false for start & ends #5', () =>
    expect(pipe.transform(text, 'Then', 'Reverbe', true)).toEqual(false));
  it('Must return boolean false for start & ends #6', () =>
    expect(pipe.transform(text, 'Then', 'Reverbe')).toEqual(false));
});
